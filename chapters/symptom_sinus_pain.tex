\chapter{Sinus Pain}

\section*{Definition}
Sinus pain (rhinosinusitis) is facial pain or pressure localized over the paranasal sinuses (maxillary, frontal, ethmoid, sphenoid) typically accompanied by nasal congestion, purulent discharge, and hyposmia lasting more than 10 days (acute) or 12 weeks (chronic).

\section*{When to See a Doctor}
See your doctor if:
\begin{itemize}
  \item Facial pain with purulent nasal discharge lasting > 10 days or worsening after 5--7 days (double sickening)
  \item Severe unilateral facial pain or dental pain (suspect odontogenic source)
  \item Periorbital swelling, proptosis, or visual changes (suspect orbital cellulitis)
  \item Headache with neurologic signs (meningitis or intracranial extension)
  \item Immunocompromised person with fever and facial pain (invasive fungal sinusitis)
\end{itemize}

\section*{How It Develops}
Ostial blockage (narrowing of sinus drainage pathways) from mucosal swelling leads to mucus accumulation, decreased oxygen tension, and impaired mucociliary clearance within the sinus cavity. Bacterial superinfection of trapped secretions triggers neutrophilic inflammation, increasing intrasinus pressure and producing characteristic pain.

\section*{Causes}
\begin{itemize}
  \item \textbf{Acute viral:} Rhinovirus, influenza, adenovirus (90\% of acute sinusitis)
  \item \textbf{Acute bacterial:} Streptococcus pneumoniae, Haemophilus influenzae, Moraxella catarrhalis
  \item \textbf{Chronic:} Persistent inflammation with biofilm formation, likely polymicrobial
  \item \textbf{Fungal:} Allergic fungal rhinosinusitis, acute invasive fungal sinusitis (immunocompromised)
  \item \textbf{Odontogenic:} Maxillary sinusitis from periapical dental collection of pus or molar root infection
  \item \textbf{Anatomic:} Septal deviation, concha bullosa, nasal polyps obstructing sinus ostia
\end{itemize}

\section*{Related Symptoms}
\begin{itemize}
  \item Facial pain/pressure (maxillary: cheek and teeth; frontal: forehead; ethmoid: medial canthus; sphenoid: vertex or occiput)
  \item Nasal congestion and blockage
  \item Purulent anterior or posterior rhinorrhea
  \item Hyposmia or loss of smell
  \item Fever, fatigue, and feeling unwell
\end{itemize}
\textbf{Important:} Isolated facial pain without nasal congestion, discharge, or endoscopic findings is more likely due to tension headache, migraine, or temporomandibular dysfunction than sinusitis.

\section*{Medical Evaluation}
\begin{itemize}
  \item Nasal endoscopy to visualize purulence from sinus ostia, mucosal swelling, and polyps
  \item CT is not routinely needed for uncomplicated acute rhinosinusitis; noncontrast CT may be used for chronic, recurrent, complicated, or diagnostically uncertain disease and for surgical planning
  \item Sinus MRI for suspected fungal sinusitis or neoplasm
  \item Cultures (endoscopic-guided swab or sinus aspirate) for refractory acute sinusitis
  \item Allergy testing if chronic sinusitis with allergic component
  \item Dental evaluation with panoramic radiograph if odontogenic source suspected
\end{itemize}

\section*{Treatment Options}
\begin{itemize}
  \item \textbf{Acute viral:} Symptomatic management only; antibiotics not indicated
  \item \textbf{Acute bacterial:} Amoxicillin-clavulanate is commonly first-line for adults when antibiotics are indicated; doxycycline is an alternative for many adults with penicillin allergy. Fluoroquinolones should be reserved for selected cases because of important adverse effects.
  \item \textbf{Chronic:} Intranasal corticosteroids, prolonged antibiotics (3--4 weeks if culture-directed), saline irrigations
  \item \textbf{Odontogenic:} Dental extraction or root canal plus anaerobic coverage (metronidazole or clindamycin)
  \item \textbf{Functional endoscopic sinus surgery (FESS):} For refractory chronic sinusitis, mucocele, or antrochoanal polyp
  \item \textbf{Balloon sinuplasty:} Minimally invasive dilation of sinus ostia (selected chronic cases)
  \item \textbf{Biologics:} Dupilumab for severe CRSwNP
\end{itemize}

\section*{Self-Care and Home Management}
\begin{itemize}
  \item Saline nasal irrigation (Neti pot) 2--4 times daily
  \item Warm compresses over the affected sinuses
  \item Steam inhalation for 10--15 minutes (bowl of hot water with towel)
  \item Over-the-counter analgesics (acetaminophen, ibuprofen) for pain
  \item Intranasal corticosteroid sprays (fluticasone, mometasone) to reduce inflammation
  \item Topical decongestant sprays (oxymetazoline) limited to 3 days to avoid rhinitis medicamentosa
  \item Oral decongestants may help selected people but can raise blood pressure and cause other adverse effects; ask a pharmacist or clinician before use.
\end{itemize}

\section*{References}
\begin{thebibliography}{99}
\bibitem{sinus_pain:uptodate-sinusitis} Rosenfeld RM, et al. Acute sinusitis: clinical features and diagnosis. In: UpToDate, Post TW (Ed), UpToDate, Waltham, MA; 2025.
\bibitem{sinus_pain:bmj-sinusitis} Ah-See KW, et al. ``Sinusitis in adults.'' \emph{BMJ}. 2023;383:e074876.
\bibitem{sinus_pain:cochrane-sinusitis} Lemiengre MB, et al. ``Antibiotics for acute sinusitis.'' \emph{Cochrane Database Syst Rev}. 2023;(7):CD014432.
\bibitem{sinus_pain:harrison-sinusitis} Longo DL, et al. \emph{Harrison\'s Principles of Internal Medicine}. 21st ed. McGraw-Hill; 2022. Chapter 134: Sinusitis.
\bibitem{sinus_pain:jam-sinusitis} Rosenfeld RM, et al. ``Clinical practice guideline for sinusitis.'' \emph{JAMA}. 2024;331(4):355-367.
\bibitem{sinus_pain:nejm-chronic-sinusitis} Orlandi RR, et al. ``Chronic Rhinosinusitis.'' \emph{N Engl J Med}. 2023;389(18):1697-1707.
\end{thebibliography}
