\chapter{Asterixis (Flapping Tremor)}

\section*{Definition}
Asterixis (also known as flapping tremor or liver flap) is a sudden, brief, arrhythmic loss of sustained muscle contraction, manifesting as a negative myoclonus. It is best elicited by having the patient dorsiflex both wrists with arms outstretched and fingers spread, producing a characteristic "flapping" motion.

\section*{Pathophysiology}
Asterixis results from temporary interruption of muscle tone due to impaired function of the reticular formation and descending motor pathways that maintain sustained posture. The pathophysiologic locus is believed to be the thalamus and midbrain, particularly the red nucleus and reticular formation, which are susceptible to metabolic toxins. Impaired cerebral energy metabolism and neurotransmitter imbalances (reduced ATP, altered dopamine/serotonin ratios) produce intermittent failure of the corticospinal and rubrospinal tracts to sustain motor unit firing.

\section*{Etiology}
\begin{itemize}
  \item \textbf{Hepatic encephalopathy:} Most common cause; seen in acute or chronic liver failure
  \item \textbf{Uremia:} Chronic kidney disease, end-stage renal disease
  \item \textbf{Respiratory failure:} Hypercapnia (CO$_2$ retention), hypoxemia
  \item \textbf{Electrolyte disturbances:} Hyponatremia, hypokalemia, hypomagnesemia, hypocalcemia
  \item \textbf{Drug-induced:} Sedatives (benzodiazepines, barbiturates), anticonvulsants (phenytoin, valproate), lithium, levetiracetam
  \item \textbf{Structural brain lesions:} Thalamic stroke, midbrain hemorrhage, bilateral basal ganglia lesions
  \item \textbf{Other metabolic:} Wilson disease, hypoglycemia, Wernicke encephalopathy
\end{itemize}

\section*{Indications for Evaluation}
Seek care if:
\begin{itemize}
  \item New-onset asterixis in a patient with known liver or kidney disease
  \item Asterixis with altered mental status, confusion, or disorientation
  \item Associated jaundice, ascites, or gastrointestinal bleeding
  \item Asterixis with fever, headache, or focal neurological deficit
\end{itemize}

\section*{Related Symptoms}
\begin{itemize}
  \item Bilaterally synchronous wrist flapping on sustained dorsiflexion
  \item May be elicited at the ankle (dorsiflexion) or by tongue protrusion
  \item Altered mental status, confusion, disorientation
  \item Fetor hepaticus (musty breath odor in liver failure)
  \item Jaundice, ascites, spider angiomata in hepatic disease
  \item Myoclonus, tremor, or other movement disorders
\end{itemize}
\textbf{Note:} Asterixis is a sign of metabolic encephalopathy, not a primary movement disorder; the underlying cause (hepatic, uremic, respiratory, or metabolic) must be identified and corrected.

\section*{Self-Care / Home Management}
\begin{itemize}
  \item Asterixis itself is not managed at home; it indicates an underlying metabolic emergency
  \item Patients with known liver disease should avoid sedatives and limit dietary protein
  \item Adherence to lactulose or rifaximin therapy in hepatic encephalopathy
  \item Maintain hydration and electrolyte balance
  \item Fall precautions due to postural instability
\end{itemize}

\section*{Diagnostic Approach}
\begin{itemize}
  \item Clinical examination: observe outstretched wrists for 30 seconds
  \item Laboratory evaluation: ammonia (hepatic), BUN/creatinine (uremic), electrolytes, glucose, LFTs, INR, ABG (respiratory), TSH
  \item Neuroimaging (CT or MRI brain) for suspected structural lesions
  \item EEG may show triphasic waves in metabolic encephalopathy
  \item Lumbar puncture if meningoencephalitis is suspected
\end{itemize}

\section*{Treatment / Management Overview}
\begin{itemize}
  \item Treatment of the underlying metabolic derangement
  \item Hepatic encephalopathy: Lactulose (titrated to 2--3 soft stools/day), rifaximin, protein restriction
  \item Uremic encephalopathy: Dialysis, optimization of renal function
  \item Respiratory failure: Correction of hypercapnia and hypoxemia
  \item Electrolyte repletion as indicated
  \item Discontinuation of causative medications
  \item Liver transplantation for end-stage liver disease with refractory encephalopathy
\end{itemize}

\clearpage
