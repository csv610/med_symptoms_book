#!/usr/bin/env python3
"""Clean up duplicate reference sections in symptom chapters."""

import os, re

CHAPTERS = "/Users/csv610/Projects/MyBooks/MedSymptoms/chapters"

# The correct references per slug (same as before)
SLUG_MAP = {
...
def build_ref_text(slug):
    """Build the reference section LaTeX code."""
    refs = REFS.get(slug)
    if not refs:
        return None
    lines = []
...
def dedup_and_fix(filepath, ref_text):
    """Remove all existing reference sections and insert one correct copy."""
    with open(filepath, 'r') as f:
        content = f.read()

    # Remove ALL existing reference sections (between \section*{References} and \end{thebibliography})
    # Use regex to handle possible variations
    pattern = r'\\section\*\{References\}\s*\n\\begin\{thebibliography\}\{99\}\n(?:.*?\n)*?\\end\{thebibliography\}\s*\n*'
    cleaned = re.sub(pattern, '', content, flags=re.DOTALL)

    if cleaned == content and r'\thebibliography' not in content:
        # No existing refs - insert at end or before \clearpage
        if r'\clearpage' in cleaned:
            new_content = cleaned.replace(r'\clearpage', ref_text.rstrip() + '\n\n' + r'\clearpage')
        else:
            new_content = cleaned.rstrip() + '\n\n' + ref_text + '\n'
        with open(filepath, 'w') as f:
            f.write(new_content)
        return "FIXED (inserted first refs)"
    elif cleaned != content:
        # Removed duplicates - now insert proper refs
        cleaned = cleaned.rstrip()
        if r'\clearpage' in cleaned:
            # Insert refs before final \clearpage
            new_content = cleaned.replace(r'\clearpage', ref_text.rstrip() + '\n\n' + r'\clearpage')
        else:
            new_content = cleaned + '\n\n' + ref_text + '\n'
        with open(filepath, 'w') as f:
            f.write(new_content)
        return "FIXED (removed {})".format(content.count(r'\thebibliography'))
    else:
        # Already has refs but pattern didn't match - try different approach
        # Just check if it has the refs we want
        return "SKIP (has refs, not modified)"


# Process all files
FILES = [
    "nasal_congestion", "nasal_polyps", "nausea_and_vomiting", "neck_mass",
    "neck_pain", "neck_stiffness", "necrotizing_fasciitis_signs", "nerve_pain",
    "night_terror", "nightmare_disorder", "nipple_discharge", "nipple_fissure", "nipple_retraction",
    "nystagmus", "obesity", "obsessive_thoughts", "oligomenorrhea", "oral_thrush",
    "orthopnea", "orthostatic_hypotension", "osgood_schlatter", "osteomyelitis_signs",
...
for slug in FILES:
    ...
    result = dedup_and_fix(filepath, ref_text)
    if "FIXED" in result:
        if slug == actual:
            fixed_exact.append(slug)
        else:
            fixed_mapped.append(f"{slug} -> {actual}")
    else:
        skipped.append(slug)
...
print("\n=== SUMMARY ===")
print(f"Fixed (exact): {len(fixed_exact)}")
print(f"Fixed (mapped): {len(fixed_mapped)}")
